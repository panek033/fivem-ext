...
void i_esp::draw_skeleton(uintptr_t ped_base) {
	...
	for (int i = 0; i < 6; ++i) {
		...
		if (bone_1 != D3DXVECTOR3(0, 0, 0) && bone_2 != D3DXVECTOR3(0, 0, 0) && ((screen_1 = i_sdk::sdk().world_to_screen(bone_1)) != D3DXVECTOR2(0, 0)) && ((screen_2 = i_sdk::sdk().world_to_screen(bone_2)) != D3DXVECTOR2(0, 0))) {
			...
			ImGui::GetForegroundDrawList()->AddLine(ImVec2(screen_1.x, screen_1.y), ImVec2(screen_2.x, screen_2.y), skeletoncolor, 1.8f);	
		}
	}
}
			...
			if (Distance <= distanceint)
			{
				...
				if (box)
				{
					if (boxtype == 0)
					{
						if (1 < Distance < 50)
						{
							ImGui::GetForegroundDrawList()->AddRect(ImVec2(Ped_Location.x - Width / 2, Ped_Location.y - Height / 2), ImVec2(Ped_Location.x + Width / 2, Ped_Location.y + Height / 2), boxcolor, 0, 0, 2.f);
						}
						if (50 < Distance < 120)
						{
							ImGui::GetForegroundDrawList()->AddRect(ImVec2(Ped_Location.x - Width / 2, Ped_Location.y - Height / 2), ImVec2(Ped_Location.x + Width / 2, Ped_Location.y + Height / 2), boxcolor, 0, 0, 1.5f);
						}
						if (120 < Distance < 500)
						{
							ImGui::GetForegroundDrawList()->AddRect(ImVec2(Ped_Location.x - Width / 2, Ped_Location.y - Height / 2), ImVec2(Ped_Location.x + Width / 2, Ped_Location.y + Height / 2), boxcolor, 0, 0, 1.f);
						}
					}
					if (boxtype == 1)
					{
						if (1 < Distance < 50)
						{
							ImGui::GetForegroundDrawList()->AddRect(ImVec2(Ped_Location.x - Width / 2, Ped_Location.y - Height / 2), ImVec2(Ped_Location.x + Width / 2, Ped_Location.y + Height / 2), boxcolor, 2.5, 0, 2.f);
						}
						if (50 < Distance < 120)
						{
							ImGui::GetForegroundDrawList()->AddRect(ImVec2(Ped_Location.x - Width / 2, Ped_Location.y - Height / 2), ImVec2(Ped_Location.x + Width / 2, Ped_Location.y + Height / 2), boxcolor, 2.5, 0, 2.f);
				...
				if (lines)
				{
					ImGui::GetForegroundDrawList()->AddLine(ImVec2(i_sdk::sdk().WindowSize.x / 2, i_sdk::sdk().WindowSize.y / 2), ImVec2(Ped_Location.x, Ped_Location.y), LineColor, 1);
					// WindowSize
				}
