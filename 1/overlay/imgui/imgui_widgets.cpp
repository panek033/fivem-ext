...
bool ImGui::Tab(const char* icon, const char* label, bool selected)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 50, pos.y + 40));
    ...
    it_hover->second = ImClamp(it_hover->second + (2.50f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 0.15f);
    ...
    it_filled->second = ImClamp(it_filled->second + (3.55f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.0f)), it_hover->second, 1.f);
    ...
    ImVec4 text = ImLerp(ImColor(100, 100, 100), ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);
    window->DrawList->AddRectFilled(pos, ImVec2(pos.x + 2, pos.y + size.y), ImColor(152, 16, 16, int(255 * it_filled->second)));
    window->DrawList->AddRectFilledMultiColor(pos, ImVec2(pos.x + 130, pos.y + size.y), ImColor(26, 26, 26, int(100 * it_filled->second)), ImColor(26, 26, 26, 0),
        ImColor(26, 26, 26, 0), ImColor(26, 26, 26, int(100 * it_filled->second)));


    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (34 / 2), bb.Min.y + (12 - label_size.y / 2)), ImColor(text), icon);
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRect(pos - ImVec2(2, 2), p + size_arg + ImVec2(2, 2), ImColor(9, 9, 9, int(155 * alpha)), 0);
    parent_window->DrawList->AddRect(pos - ImVec2(1, 1), p + size_arg + ImVec2(1, 1), ImColor(10, 10, 10, int(155 * alpha)), 0);
    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(10, 10, 10, int(255 * alpha)), 0);
    parent_window->DrawList->AddRectFilled(pos, p + ImVec2(270, 8), ImColor(10, 10, 10, int(255 * alpha)), 0);

    parent_window->DrawList->AddRect(pos, p + size_arg, ImColor(24, 24, 24, int(255 * alpha)), 0);


    parent_window->DrawList->AddText(pos + ImVec2(6, 3), ImColor(255, 255, 255), name);



    parent_window->DrawList->AddRectFilledMultiColor(pos, p + ImVec2(270, 8), ImColor(18, 18, 18, 180), ImColor(18, 18, 18, 0), ImColor(18, 18, 18, 0), ImColor(18, 18, 18, 180));

    parent_window->DrawList->AddLine(p + ImVec2(1, 8), p + ImVec2(200, 8), ImColor(24, 24, 24, int(255 * alpha)));
    return ret;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (mixed_value)
    {
        ...
        RenderFrame(check2_bb.Min, check2_bb.Max, (held && hovered) ? ImColor(115,115,115) : hovered ? ImColor(105, 255, 255) : ImColor(115, 115, 255), false, 2.0f);
        const ImVec2 line_size = ImVec2(check2_bb.GetWidth() / 2 - style.ItemInnerSpacing.x, 1.5f);
        RenderFrame(check2_bb.GetCenter() - line_size, check2_bb.GetCenter() + line_size, ImColor(255, 255, 255), false, 2.0f);
    }
    else if (*v)
    {
        ImU32 frame_col = (held && hovered) ? ImColor(105, 105, 105) : (hovered ? ImColor(85, 85, 85) : ImColor(85, 85, 85));
        RenderFrame(check2_bb.Min, check2_bb.Max, frame_col, false, 2.0f);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), ImColor(215, 215, 215), square_sz - pad * 2.0f);
    }
    else {
        ImU32 border_col = (held && hovered) ? ImColor(85, 85, 85) : (hovered ? ImColor(55, 55, 55) : ImColor(55, 55, 55));
        PushStyleVar(ImGuiStyleVar_FrameBorderSize, 2.0f);
        PushStyleColor(ImGuiCol_Border, border_col);
        RenderFrameBorder(check2_bb.Min, check2_bb.Max, 2.0f);
        PopStyleVar();
        PopStyleColor();
    }
    ...
    if (label_size.x > 0.0f)
        RenderText(label_pos, label, nullptr, true, hovered ? ImColor(255, 255, 255) : ImColor(255, 255, 255));
...
bool ImGui::SecondChild(const char* str_id, const ImVec2& size_arg, bool sub_tab, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return Secondchildex(str_id, window->GetID(str_id), size_arg, sub_tab, extra_flags);
}


bool ImGui::FirstChild(const char* str_id, const ImVec2& size_arg, bool sub_tab, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return Firstchildex(str_id, window->GetID(str_id), size_arg, sub_tab, extra_flags);
}



bool ImGui::Firstchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    ...
    const int auto_fit_axises = ((size.x == 0.0f) ? (1 << ImGuiAxis_X) : 0x00) | ((size.y == 0.0f) ? (1 << ImGuiAxis_Y) : 0x00);
    if (size.x <= 0.0f)
        size.x = ImMax(content_avail.x + size.x, 1.0f); // Arbitrary minimum child size (0.0f causing too much issues)
    if (size.y <= 0.0f)
        size.y = ImMax(content_avail.y + size.y, 4.0f);
    ...
    // Set the cursor to handle case where the user called SetNextWindowPos()+BeginChild() manually.
    // While this is not really documented/defined, it seems that the expected thing to do.
    if (child_window->BeginCount == 0)
        parent_window->DC.CursorPos = child_window->Pos;

    child_window->DC.CursorPos += ImVec2(1, 10);
    // Process navigation-in immediately so NavInit can run on first frame




    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImVec2(size_arg.x, size_arg.y), ImColor(19, 19, 19, (int)(GetStyle().Alpha * 255.f)), 0, ImDrawCornerFlags_Bot);
...
bool ImGui::Secondchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    ...
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(0, 28), ImGui::GetWindowPos() + size_arg, ImColor(19, 19, 19, int(255 * g.Style.Alpha)), 6);
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(0, 28), ImGui::GetWindowPos() + size_arg, ImColor(19, 19, 19, int(255 * g.Style.Alpha)), 6);
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() - ImVec2(0, 32), ImGui::GetWindowPos() + ImVec2(size_arg.x, 0), ImColor(23, 23, 23, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Top);
    parent_window->DrawList->AddLine(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImVec2(size_arg.x, 0), ImColor(39, 40, 41, int(255 * g.Style.Alpha)), 1);
    parent_window->DrawList->PushClipRect(ImGui::GetWindowPos() - ImVec2(0, 32), ImGui::GetWindowPos() + ImVec2(size_arg.x, 0), false);
    ...
    parent_window->DrawList->AddText({ ImGui::GetWindowPos().x + 180,ImGui::GetWindowPos().y - (16 + CalcTextSize(name).y / 2) }, ImColor(255, 255, 255, 200), name);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(15, 15, 15), 3);

    window->DrawList->AddText(rect.Min + ImVec2(5, -5), ImColor(255, 255, 255, 200), label);
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(1, 0)); // Horizontally align ourselves with the framed text
    PushStyleVar(ImGuiStyleVar_PopupRounding, 0);
    PushStyleVar(ImGuiStyleVar_PopupBorderSize, 1.0f);
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(15, 15, 15)));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    // Calculate bounds
    const float grab_padding = 1.0f; // FIXME: Should be part of style.
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos + ImVec2(15, -10), window->DC.CursorPos + ImVec2(w, label_size.y + 5));
    ...
    // Draw f const bool clicked = (hovered && g.IO.MouseClicked[0]);rame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ...
    it_anim->second.outline_anim = ImLerp(it_anim->second.outline_anim, hovered || IsItemActive() ? 0.1f : 0.0f, 0.04f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 21, frame_bb.Max.y), ImColor(202, 202, 202), 2); //(0.64f, 0.62f, 0.63f, 1.00f); //
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.0f, 1.0f, 1.0f, 0.05f + it_anim->second.outline_anim), 2);

    RenderText(total_bb.Min + ImVec2(0, 2), label);
    RenderText(total_bb.Min + ImVec2(197, 2), value_buf);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        window->DrawList->AddRectFilled(pos, ImVec2(pos.x + 2, pos.y + size.y + 0), ImColor(157, 176, 244, int(255)));
        window->DrawList->AddRectFilledMultiColor(pos, ImVec2(pos.x + 130, pos.y + size.y + 0), ImColor(26, 26, 26, int(100)), ImColor(26, 26, 26, 0),
            ImColor(26, 26, 26, 0), ImColor(26, 26, 26, int(100)));
    }
